...
export default function CardTable({ color }) {
  ...
  const handleSuspendBooking = (index) => {
    ...
    if (existingBookings[index]) {
      existingBookings[index].status = "Suspended";
    ...
    history.push("/supperadmin/admin"); // Replace '/dashboard' with the desired URL
  };

  const handleActiveBooking = (index) => {
    // Get the existing bookings from local storage
    const existingBookings = JSON.parse(localStorage.getItem("bookings")) || [];
                  ...
                  <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-right">
                    {booking.status === "Suspended" ? (
                      <button
                        ...
                        type="button"
                        onClick={() => handleActiveBooking(index)}
                      >
                        Activate
                      </button>
                    ) : (
                    ...
                    {booking.status === "Booked" ? (
                      <div>
                        <button
                          className="bg-black text-white active:bg-gray-700 font-bold uppercase text-xs px-4 py-2 rounded-full outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150"
                          type="button"
                          onClick={() => handleSuspendBooking(index)}
                        >
                          Suspend
                        </button>
                        <button
